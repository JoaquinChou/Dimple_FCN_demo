...
def main(opt):
    ...
    img_cv2 = cv2.imdecode(np.fromfile(img_pth, dtype=np.uint8), -1)
    img_torch = torch.from_numpy((img_cv2 - img_cv2.mean()) / (img_cv2.std() + 1e-5)[np.newaxis, np.newaxis]).float()
    ...
    cv2.imwrite('pred.png', img_cv2)


if __name__ == '__main__':

    opt = Opt()
    opt.img_pth = '/home3/wjx/Dimple/dataset/非Dimple/8. 红色检测框外，边缘区域有Dimple/[000070]XU02_SUB_L_171021.jpg'
    opt.gpu = 1
    opt.step = 1000
