...
class Resnet18_Backbone(Resnet18):
    def __init__(self, *args, **kwargs):
        super(Resnet18_Backbone, self).__init__(*args, **kwargs)
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
         # MLP for SupCon
        self.mlp = nn.Sequential(nn.Linear(512, 512),
                                nn.ReLU(inplace=True),
                                nn.Linear(512, 128))
    ...
    def forward(self, x):
            ...
            return features, x, seg_features
...
class LinearClassifier(nn.Module):
    """Linear classifier"""
    def __init__(self, name='Resnet18', cls_classes=None):
        super(LinearClassifier, self).__init__()
        self.fc = nn.Linear(512, cls_classes)
...
class FCN_res18(_SimpleSegmentationModel):
    def __init__(self, num_classes=1, cls_classes=None):
        super(FCN_res18, self).__init__(
            # backbone=Resnet18(num_classes=num_classes),
        ...
        # self.supcon_classifier=LinearClassifier(num_classes=1)
        self.supcon_classifier=LinearClassifier(cls_classes=cls_classes)

    def forward(self, x):
        input_shape = x.shape[-2:]
        # features, cls_logits = self.backbone(x)
        cls_features, _, seg_features = self.backbone(x)
        # 分类
        cls_logits = self.supcon_classifier(cls_features)
        # 分割
        # seg_logits = self.classifier(features)
        seg_logits = self.classifier(seg_features)
        seg_logits = F.interpolate(seg_logits, size=input_shape, mode='bilinear', align_corners=False)
        # return cls_logits, seg_logits
