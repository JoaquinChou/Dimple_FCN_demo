...
parser.add_argument('--output_anormal', default=False, type=bool, help='output anormality or not')
parser.add_argument('--img_path_name', default=None, type=str, help='img_path_name')
...
if args.output_anormal:
        img_path_list = []
        print("Done Output Anormality!")
        with open(res_dir + args.results_txt.split('_')[0] + '/' + args.img_path_name, encoding='utf-8') as f:
                img_path_list = f.readlines()

        clf = KNeighborsClassifier(n_neighbors=5)  # n_neighbors 为设定K的值
        clf.fit(digits_data, digits_target)
        X_tsne_predict = clf.predict(digits_data)
        # 有噪声的图像为
        for i in range(len(X_tsne_predict)):
                if X_tsne_predict[i] != digits_target[i]:
